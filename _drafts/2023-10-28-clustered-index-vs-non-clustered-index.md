...
# 클러스터드 인덱스
클러스터드 인덱스가 설정된 테이블은 기본적으로 파티션이 하나다. 그 하나의 파티션 안에 B-Tree 구조의 인덱스가 설정된 것이다. 만약 테이블에 파티션이 지정되면 그 파티션 마다 B-Tree 구조가 설정된다. 예를들어 Date형식의 컬럼에 연도마다 파티션이 구성된다면, 매해마다 새로운 B-Tree 구조의 인덱스가 설정되는 것이다.
